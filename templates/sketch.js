...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (result) {
    console.log("Result element found:", result.innerText);
  } else {
    console.log("Result element not found yet");
  }
});
...
async function modelReady() {
  ...
  try {
    ...
    // Try to fix the model.weights.bin path issue
    if (error.message && error.message.includes('model.weights.bin') || error instanceof RangeError) {
      console.log('Attempting to load model with corrected weights path...');
      await classifier.load('./model.json', {
        fetchFunc: (path, init) => {
          // If the path contains the problematic path structure, fix it
          if (path.includes('model.weights.bin')) {
            console.log('Correcting weights path from:', path);
            // Try different path resolutions
            const possiblePaths = [
              './model.weights.bin',
              '../templates/model.weights.bin',
              '/templates/model.weights.bin'
            ];
            
            // Use the first path that exists
            console.log('Trying alternative paths for weights file');
            return fetch(possiblePaths[0], init)
              .catch(() => fetch(possiblePaths[1], init))
              .catch(() => fetch(possiblePaths[2], init));
          }
          return fetch(path, init);
        }
      }, function() {
        select('#modelStatus').html('Custom Model Loaded with path correction!');
      });
    }
  }
}
